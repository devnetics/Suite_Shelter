...
export class MapPage {
  private _heatmap;
  private _map;
  constructor(public confData: ConferenceData, public http: Http) {}
  ...
  getPoints() {
    this.http.get('http://localhost:8080/heatmap')
                      .subscribe(result => {
                        ...
                        let i: any;
                        for (i in results) {
                          heat_map.push(new google.maps.LatLng(i.latitude, i.longitude))
                        }
                        console.log(heat_map);
                        this._heatmap.setData(heat_map);
                      });
  }
  ...
  ionViewDidLoad() {
    this.confData.getMap().then(mapData => {
      ...
      this._map = new google.maps.Map(mapEle, {
        zoom: 12,
        center: {lat: 37.775, lng: -122.434},                
        mapTypeId: 'satellite'
      });    
      this._heatmap = new google.maps.visualization.HeatmapLayer({
          data: [new google.maps.LatLng(37.782551, -122.445368),
